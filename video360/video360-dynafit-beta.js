...
function onMouseMove(event)
{
    ...
    {       
        ...
        a = a + moveX;
        document.getElementById("info").innerHTML = angle[a];
        // player.pauseVideo();
...
function initialize()
{
    ...
    for (var i = 0; i < n; i++) {
        stopPoints[i] = clipDuration * (i+1) + 4/fps;
    }
...
function stopVideo()
{
    for (var i = 0; i < n; i++) 
    {
        if(player.getCurrentTime() > (stopPoints[i] - (1/fps)) && player.getCurrentTime() < (stopPoints[i] + (1/fps)))
        {
            player.pauseVideo();
...
function loopVideo()
{
    for (var i = 0; i < n; i++) 
    {
        ...
        {
            L = i;
            document.getElementById("loop").innerHTML = "LOOP: " + L + " / loopPOINTS: " + loopPoints[L] + " / stopPOINTS: " + stopPoints[L];           
            break;
        }
    }    
    if(L != null)
    {        
        ...
        player.seekTo(loopPoints[L], true);
        L = null;
    }
}

var Control = document.getElementById("stop");
Control.addEventListener('mousedown', this.stopplay, false);
Control.addEventListener("touchstart", this.stopplay, false);
